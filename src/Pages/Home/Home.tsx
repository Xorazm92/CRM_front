...
function Home() {
  ...
  const [latestStudents, setLatestStudents] = useState<Student[]>([]); // Agar kerak bo‘lsa, dashboard/recent-payments dan foydalaniladi
  const [latestPayments, setLatestPayments] = useState<Payment[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string>("");
  const [ageStats, setAgeStats] = useState<AgeStats>({ labels: [], data: [] });
  ...
  useEffect(() => {
    // Umumiy statistika (o‘quvchilar, o‘qituvchilar, guruhlar va h.k.)
    const fetchDashboardStats = async () => {
      setLoading(true);
      setError("");
      try {
        const statsRes = await instance.get("/dashboard/stats");
    ...
    const fetchRecentPayments = async () => {
      try {
        const paymentsRes = await instance.get("/dashboard/recent-payments");
        const payments = Array.isArray(paymentsRes.data) ? paymentsRes.data : paymentsRes.data.results || [];
        setLatestPayments(payments.slice(0, 5));
      } catch (err) {
        // Errorni hozircha ko‘rsatmaymiz
      }
    };
    fetchRecentPayments();

    // Bolalarning yoshi bo‘yicha statistikasi
    const fetchAgeStats = async () => {
      setAgeLoading(true);
      setAgeError("");
      try {
        const res = await instance.get("/dashboard/students/age-stats");
            ...
            <table className="home-table">
              ...
              <tbody>
                {latestStudents.length === 0 ? (
                  ...
                  latestStudents.slice(0,3).map((s: Student, i: number) => (
                    <tr key={s._id || i}>
                      <td>{i + 1}</td>
                      <td>
            ...
            <table className="home-table">
              ...
              <tbody>
                {latestStudents.length === 0 ? (
                  ...
                  latestStudents.slice(0,4).map((s: Student, i: number) => (
                    <tr key={s._id || i}>
                      <td>{i + 1}</td>
                      <td>
                        <span style={{display:'flex',alignItems:'center',gap:8}}>
